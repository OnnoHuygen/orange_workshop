...
\title{Orange workshop - SMS dataset}
\author{Onno Huijgen}
\date{\today}
...
\section*{Introductie}
Deze opdracht is onderdeel van de Orange Data Mining workshop. Al het matieraal is te vinden via Github: \url{https://github.com/OnnoHuygen/orange_workshop/}.
...
\item Dit keer is de dataset niet al aanwezig in de tool, en moeten we hem zelf inladen. Maak een module \tb{file} aan op je canvas en laad de SMS-data met labels in. Zorg dat de feature SPAM als \textit{categorical target} variabele staat gemarkeerd (zie figuur~\ref{fig:input}).
...
\item Nu de data gestructureerd in een \tb{Corpus} staat, kunnen we de tekst bekijken met de \verb|Corpus viewer| module, zie figuur~\ref{fig:cvv}.
...
\begin{figure}[hbtp]
  ...
  \begin{subfigure}[b]{0.5\textwidth}
    \centering
    \includegraphics[width=0.99\textwidth]{Figures/input.png}
    \caption{Menu input data.}
    \label{fig:input}
  \end{subfigure}%
  ~ 
  \begin{subfigure}[b]{0.5\textwidth}
    \centering
    \includegraphics[width=0.99\textwidth]{Figures/corpus_view.png}
    \caption{Corpus viewer}
    \label{fig:cvv}
  \end{subfigure}
  \caption{}
  \label{fig:cv}
...
Vaak is het niet een goed idee om de data direct zoals hij is in een machine learning model te stoppen. Het is voor een model belangrijk dat de data in het juiste formaat staat, dat er geen missende waardes zijn, en dat er de juiste features zijn waar het model ook van kan leren.
...
\item In de preprocessor kun je verschillende preprocessing stappen tegelijk uit laten voeren. In figuur~\ref{fig:preprocess} kun je zien welke stappen standaard aanwezig zijn. Zorg er allereerst voor dat de preprocessor alle woorden in kleine letters zet.
\item Zorg dat je preprocessor alle smsjes opbreekt in losse woorden of woordonderdelen. Dit heet tokenization.
...
Je pipeline zou er nu ongeveer uit moeten zien als figuur~\ref{fig:pip1}
\begin{figure}[hbtp]
  \centering
  \begin{subfigure}[b]{0.5\textwidth}
    \centering
    \includegraphics[width=0.99\textwidth]{Figures/preprocess_text.png}
    \caption{Text preprocessing module die meerdere stappen uitvoert.}
    \label{fig:preprocess}
  \end{subfigure}%
  ~ 
  \begin{subfigure}[b]{0.5\textwidth}
    \centering
    \includegraphics[width=0.99\textwidth]{Figures/pip1.png}
    \caption{Pipeline after preprocessing}
    \label{fig:pip1}
  \end{subfigure}
  \caption{}
  \label{fig:pip}
...
In deze oefening heb je data ge\"importeerd, schoongemaakt en features ge-engineered. Je hebt meerdere machine learning modellen getraind en de resultaten ge\"evalueeerd. Daarnaast heb je modellen samengevoegd in een \textit{ensemble}. Vevolgens heb je je modellen toegepast op nieuwe, ongeziene data. Dit zijn de stappen die doorgaans doorlopen worden in de machine learning pipeline, en je kunt met dezelfde methode heel veel verschillende datasets aanvliegen. De uiteindelijke pipeline zal er ongeveer uitzien zoals in figuur~\ref{fig:pip2}

\begin{figure}[hbtp]
  \centering
  \includegraphics[width=0.9\textwidth]{Figures/pip2.png}
  \caption{Uiteindelijke pipeline voor de spamfilter}
  \label{fig:pip2}
\end{figure}
